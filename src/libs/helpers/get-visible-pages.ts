...
type GetVisiblePagesType = (pages: number[], currentPage: number) => number[];
...
export const getVisiblePages: GetVisiblePagesType = (pages, currentPage) => {
  ...
  if (currentPage - VISIBLE_PAGES_COUNT < firstPage - 1) {
    return [...pages.slice(0, VISIBLE_PAGES_COUNT), lastPage];
  }

  if (currentPage + VISIBLE_PAGES_COUNT > lastPage + 1) {
    return [firstPage, ...pages.slice(-VISIBLE_PAGES_COUNT)];
  }
